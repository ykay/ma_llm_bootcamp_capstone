...
- A "student aid" service that extracts key takeaways and things to do, by providing an LLM that evolves over time as it accumulates knowledge from past lectures and course material. The idea is that something like this would be used in realtime, so it becomes a dashboard that students check into after each class to review notes and the video recording.

## Demo Presentation
The demo is made up of the following:

- FastAPI server serving LLM Bootcamp API
- Next.js frontend app (calls LLM Bootcamp API)
